...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="les1996/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url:'https://github.com/Lee-es/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage ('JMeter LoadTest') {
            steps { 
                sh '~/lab/sw/apache-jmeter-5.6.3/bin/jmeter.sh -j jmeter.save.saveservice.output_format=xml -n -t src/main/jmx/guestbook_loadtest.jmx -l loadtest_result.jtl' 
